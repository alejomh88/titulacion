...
@Controller
@Scope("session")
public class RecepcionDETTBean implements Serializable {
	...
	public void guardar() {
		...
		EstudianteTrabajoTitulacion ett2 = estudianteTrabajoTitulacionService.obtenerObjeto(
				"select tt from EstudianteTrabajoTitulacion tt inner join tt.seminarioTrabajoTitulacion stt where stt.id=?1",
				new Object[] { estudianteTrabajoTitulacion.getSeminarioTrabajoTitulacion().getId() }, false,
				new Object[] {});
		if (ett2 != null && ett2.getId() != null) {
			ett2.setRecepcionDE(true);
