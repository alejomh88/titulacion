...
@Controller
@Scope("session")
public class CalificacionTemaPracticoDOCEBean implements Serializable {
	...
	private List<Usuario> listDocentes;
	...
	public void buscar() {
		listTemaPracticos = temaPracticoService.obtenerLista(
				"select distinct tp from TemaPractico tp inner join tp.estudiantePeriodo ep inner join ep.estudiante e inner join ep.periodoExamen pe inner join pe.proceso p inner join pe.carrera c "
						...
						+ "and esp1.email=?3 and (esp1.id=?4 or esp2.id=?4 or esp3.id=?4) order by tp.fechaSustentacionOrdinaria ",
				new Object[] { carrera, UtilsDate.timestamp(),
	...
	public void cargarDocentes() {
		String cedula = usuarioService.obtenerObjeto("select u from Usuario u where u.email=?1",
				new Object[] { SecurityContextHolder.getContext().getAuthentication().getName().trim().toLowerCase() },
				false, new Object[0]).getId();
		listDocentes = usuarioService.obtenerPorSql("SELECT distinct u.* FROM exetasi.usuarios u "
				+ "inner join exetasi.\"temasPracticos\" tp on(u.id=tp.especialista1 or u.id=tp.especialista2 or u.id=tp.especialista3) "
				...
				+ "inner join exetasi.carreras c on(pe.carrera=c.id) where tp.especialista1='" + cedula + "' and c.id="
				+ carrera + " order by u.\"apellidoPaterno\", u.\"apellidoMaterno\", u.nombre", Usuario.class);
	}
	...
	public List<Usuario> getListDocentes() {
		return listDocentes;
	}
	...
	public void init() {
		...
		docente = usuarioService
				.obtenerObjeto("select u from Usuario u where u.email=?1", new Object[] { email }, false, new Object[0])
				.getId();
		listCarreras = carreraService.obtenerLista(
				"select distinct c from Carrera c inner join c.peridosExamenes pe inner join pe.estudiantesPeriodos ep inner join ep.temasPracticos tp "
						+ "inner join tp.especialista1 esp1 where esp1.email=?1 order by c.nombre",
		...
		if (bn == false)
			...
			for (CalificacionAux cAux : listCalificacion) {
				...
				c.setEspecialista(usuarioService.obtenerObjeto("select u from Usuario u where u.id=?1",
						new Object[] { docente }, false, new Object[0]));
	...
	public void limpiarEsc(EstudianteExamenComplexivoPP temaPractico) {
		...
		if (listCalificacionAux != null && !listCalificacionAux.isEmpty())
			UtilsAplicacion.presentaMensaje(FacesMessage.SEVERITY_INFO,
					"Ya Ingreso la calificación de este estudiante");
		else {
			especialista = usuarioService.obtenerObjeto("select u from Usuario u where u.id=?1",
					new Object[] { docente }, false, new Object[0]);
	...
	public void limpiarOral(EstudianteExamenComplexivoPP temaPractico) {
		this.temaPractico = temaPractico;
		List<Calificacion> listCalificacionAux = calificacionService.obtenerListaDirecta(
				"select c from Calificacion c inner join c.especialista e inner join c.temaPractico tp "
						+ "inner join c.tipoExamen te inner join c.indicador i "
	...
	public void setListDocentes(List<Usuario> listDocentes) {
