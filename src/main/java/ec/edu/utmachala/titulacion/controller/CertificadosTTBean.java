...
@Controller
@Scope("session")
public class CertificadosTTBean implements Serializable {
	...
	private EstudianteTrabajoTitulacion estudianteETTSeleccionado;
	...
	public void a() {
		...
		certificadosEstudiantes = new ArrayList<CertificadoEstudiante>();
	...
	public void buscar() {
		listadoEstudiantesTrabajosTitulacion = new ArrayList<EstudianteTrabajoTitulacion>();
		if (criterioBusquedaEstudiante.isEmpty())
			listadoEstudiantesTrabajosTitulacion = estudiantesTrabajoTitulacionService.obtenerLista(
					"select ett from EstudianteTrabajoTitulacion ett inner join ett.proceso p inner join fetch ett.carrera c inner join ett.estudiante e "
							+ "where c.id=?1 and p.id=?2 and ett.numeroActaCalificacion is not null order by e.apellidoNombre",
			...
			listadoEstudiantesTrabajosTitulacion = estudiantesTrabajoTitulacionService.obtenerLista(
					"select ett from EstudianteTrabajoTitulacion ett inner join ett.proceso p inner join fetch ett.carrera c "
							...
							+ "and ett.numeroActaCalificacion is not null order by e.apellidoNombre",
	...
	public EstudianteTrabajoTitulacion getEstudianteETTSeleccionado() {
		return estudianteETTSeleccionado;
	}
	...
	public void guardarAdeudar(CertificadoEstudiante certificadoEstudianteSeleccionado) {
		...
		if (certificadoEstudianteSeleccionado.getAdeuda()) {
			...
			if (certificadoEstudianteSeleccionado.getCertificado().getDependencia().getId()
				...
				String destinatario = certificadoEstudianteSeleccionado.getEstudianteTrabajoTitulacion().getEstudiante()
						.getEmail();
	...
	public void obtenerCarreras() {
		carreras = carreraService.obtenerLista(
				"select distinct c from Carrera c inner join c.permisosCarreras pc inner join pc.usuario u "
						...
						+ "where u.email=?1 and p.id=?2 and tt.numeroActaCalificacion is not null order by c.nombre",
				new Object[] { SecurityContextHolder.getContext().getAuthentication().getName().trim().toLowerCase(),
						proceso },
				0, false, new Object[] {});

		if (listadoEstudiantesTrabajosTitulacion != null)
			listadoEstudiantesTrabajosTitulacion.clear();
		...
		try {
			certificadosEstudiantesNoAdeudar = certificadoEstudianteService.obtenerLista(
					"select distinct ce from CertificadoEstudiante ce inner join ce.certificado cer "
							...
							+ "inner join ce.estudianteTrabajoTitulacion ett inner join ett.proceso p "
	...
	public void onRowSelect() {
		System.out.println("id del eppSeleccionado: " + estudianteETTSeleccionado.getId());

		certificadosEstudiantes = certificadoEstudianteService
				.obtenerPorSql("select distinct ce.* from carreras c inner join"
						+ " \"estudiantesTrabajosTitulacion\" ett on (ett.carrera=c.id) inner join"
						+ " usuarios e on (ett.estudiante=e.id) inner join"
						+ " procesos p on (ett.proceso=p.id) inner join"
						+ "	\"certificadosEstudiantes\" ce on (ce.ett=ett.id) inner join"
						...
						+ " and u.email = '" + UtilSeguridad.obtenerUsuario() + "' and ett.id="
						+ estudianteETTSeleccionado.getId() + " order by ce.id;", CertificadoEstudiante.class);
	...
	public void setEstudianteETTSeleccionado(EstudianteTrabajoTitulacion estudianteETTSeleccionado) {
		this.estudianteETTSeleccionado = estudianteETTSeleccionado;
	}
